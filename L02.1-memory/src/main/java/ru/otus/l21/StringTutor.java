...
public class StringTutor {
    ...
    private static final String greetingRegex = "^Привет,\\s*[А-ЯЁ][-А-яЁё]{2,}\\s*[А-ЯЁ][-А-яЁё]{2,}\\s*!$";
    Pattern greetingPattern = Pattern.compile(greetingRegex);
    ...
    public boolean checkGreeting(String greeting) {
        if (!this.isCorrectGreeting(greeting)) {
            return false;
        }
        return true;
    }

    private boolean isCorrectGreeting(String testString) {
        Matcher m = greetingPattern.matcher(testString);
        return m.matches();
    }
